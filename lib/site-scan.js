...
async function takeScreenshot(url, page, fileName) {
  ...
  try {
    host = await page.evaluate(() => Promise.resolve(window.location.host));
  } catch(e) {
    console.log(chalk`{red • Error reading from ${url} due to possible re-direct. Try using the --sleep option to fix this! Skipping...}`);
    return;
  }
